-- Working version of get_trainer_users function
-- This script should be run in Supabase SQL Editor

-- Drop the existing function first
DROP FUNCTION IF EXISTS get_trainer_users(VARCHAR(50));

-- Create a working function that gets users with their first session info
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        up.email,
        COALESCE(
            (SELECT (jsonb_array_elements(pts.schedule_data->'sessions')->>'date')::TEXT 
             FROM jsonb_array_elements(pts.schedule_data->'sessions') 
             WHERE (jsonb_array_elements(pts.schedule_data->'sessions')->>'trainer') = trainer_name_param 
             LIMIT 1), 
            ''
        ) as next_session_date,
        COALESCE(
            (SELECT (jsonb_array_elements(pts.schedule_data->'sessions')->>'startTime')::TEXT 
             FROM jsonb_array_elements(pts.schedule_data->'sessions') 
             WHERE (jsonb_array_elements(pts.schedule_data->'sessions')->>'trainer') = trainer_name_param 
             LIMIT 1), 
            ''
        ...
        COALESCE(
            (SELECT (jsonb_array_elements(pts.schedule_data->'sessions')->>'type')::TEXT 
             FROM jsonb_array_elements(pts.schedule_data->'sessions') 
             WHERE (jsonb_array_elements(pts.schedule_data->'sessions')->>'trainer') = trainer_name_param 
             LIMIT 1), 
            ''
        ...
        COALESCE(
            (SELECT (jsonb_array_elements(pts.schedule_data->'sessions')->>'room')::TEXT 
             FROM jsonb_array_elements(pts.schedule_data->'sessions') 
             WHERE (jsonb_array_elements(pts.schedule_data->'sessions')->>'trainer') = trainer_name_param 
             LIMIT 1), 
            ''
...
-- Test the function
SELECT 'Testing working get_trainer_users function...' as status;
