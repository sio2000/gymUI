-- Create Pilates Package Only
-- This script just creates the Pilates package and its durations

-- Step 1: Check if Pilates package already exists
SELECT 'Checking for existing Pilates package...' as info;
SELECT id, name, package_type FROM membership_packages WHERE name = 'Pilates';

-- Step 2: Create Pilates package if it doesn't exist
...
-- Step 3: Get the Pilates package ID
SELECT 'Pilates package created/found:' as info;
SELECT id, name, package_type FROM membership_packages WHERE name = 'Pilates';

-- Step 4: Create duration options
...
SELECT 'Final verification - Pilates package and durations:' as info;
SELECT 
    mp.id,
    mp.name,
