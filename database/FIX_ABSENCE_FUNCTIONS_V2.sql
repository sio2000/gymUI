-- Fix absence system functions - Version 2
...
-- Fixed function to get users assigned to a specific trainer
-- This version uses a simpler approach without LATERAL
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        up.email,
        COALESCE(
            (SELECT (jsonb_array_elements(pts2.schedule_data->'sessions')->>'date')
             FROM personal_training_schedules pts2 
             WHERE pts2.user_id = up.user_id 
             AND (jsonb_array_elements(pts2.schedule_data->'sessions')->>'trainer') = trainer_name_param
             LIMIT 1), 
            ''
        ) as next_session_date,
        COALESCE(
            (SELECT (jsonb_array_elements(pts2.schedule_data->'sessions')->>'startTime')
             FROM personal_training_schedules pts2 
             WHERE pts2.user_id = up.user_id 
             AND (jsonb_array_elements(pts2.schedule_data->'sessions')->>'trainer') = trainer_name_param
             LIMIT 1), 
        ...
        COALESCE(
            (SELECT (jsonb_array_elements(pts2.schedule_data->'sessions')->>'type')
             FROM personal_training_schedules pts2 
             WHERE pts2.user_id = up.user_id 
             AND (jsonb_array_elements(pts2.schedule_data->'sessions')->>'trainer') = trainer_name_param
             LIMIT 1), 
        ...
        COALESCE(
            (SELECT (jsonb_array_elements(pts2.schedule_data->'sessions')->>'room')
             FROM personal_training_schedules pts2 
             WHERE pts2.user_id = up.user_id 
             AND (jsonb_array_elements(pts2.schedule_data->'sessions')->>'trainer') = trainer_name_param
             LIMIT 1), 
        ...
        COALESCE(
            (SELECT jsonb_array_length(pts2.schedule_data->'sessions')
             FROM personal_training_schedules pts2 
             WHERE pts2.user_id = up.user_id 
             AND (jsonb_array_elements(pts2.schedule_data->'sessions')->>'trainer') = trainer_name_param
             LIMIT 1), 
