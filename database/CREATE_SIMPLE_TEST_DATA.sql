-- CREATE SIMPLE TEST DATA FOR QR CATEGORIES
-- This script creates test data that works with the actual database structure
...
BEGIN
    ...
    -- Get package IDs
    SELECT id INTO free_gym_package_id FROM membership_packages WHERE package_type = 'free_gym' AND is_active = true LIMIT 1;
    SELECT id INTO pilates_package_id FROM membership_packages WHERE package_type = 'pilates' AND is_active = true LIMIT 1;
    SELECT id INTO personal_package_id FROM membership_packages WHERE package_type = 'personal_training' AND is_active = true LIMIT 1;
    
    -- If personal_training doesn't exist, try 'personal'
    IF personal_package_id IS NULL THEN
        SELECT id INTO personal_package_id FROM membership_packages WHERE package_type = 'personal' AND is_active = true LIMIT 1;
    END IF;
