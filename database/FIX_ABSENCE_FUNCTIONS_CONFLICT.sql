/* FIX ABSENCE FUNCTIONS CONFLICT - ΔΙΟΡΘΩΣΗ CONFLICT ΣΤΑ FUNCTIONS
   Εκτέλεση στο Supabase SQL Editor */

/* 1. Check existing functions */
SELECT 'Existing functions check:' as step;
SELECT 
    routine_name,
    routine_type,
    data_type
...
AND routine_name IN ('get_trainer_users', 'get_user_absences', 'add_absence', 'update_absence', 'delete_absence')
ORDER BY routine_name;

/* 2. Drop ALL existing functions with CASCADE to handle dependencies */
DROP FUNCTION IF EXISTS get_trainer_users(TEXT) CASCADE;
DROP FUNCTION IF EXISTS get_trainer_users(VARCHAR(50)) CASCADE;
DROP FUNCTION IF EXISTS get_user_absences(UUID, TEXT) CASCADE;
DROP FUNCTION IF EXISTS get_user_absences(UUID, VARCHAR(50)) CASCADE;
...
DROP FUNCTION IF EXISTS update_absence(UUID, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS update_absence(UUID, VARCHAR(20), TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS delete_absence(UUID) CASCADE;

/* 3. Verify functions are dropped */
SELECT 'Functions after drop:' as step;
SELECT 
    routine_name,
    routine_type
FROM information_schema.routines 
...
/* 4. Create get_trainer_users function with correct signature */
...
/* 5. Create get_user_absences function */
...
/* 7. Create update_absence function */
...
/* 8. Create delete_absence function */
...
/* 9. Verify all functions are created */
SELECT 'Final functions check:' as step;
SELECT 
    routine_name,
    routine_type,
    data_type
...
/* Success message */
SELECT 'All absence functions fixed and working!' as message;
